  ...
  const addBlock = (type: BlockItem['type']) => {
    ...
    switch (type) {
      ...
      case 'governance_structure':
        defaultData = { badge: "Skema Hierarki", title: "Struktur Organisasi", desc: "Garis komando pertanggungjawaban mutlak.", topBadge: "Supervisi", topTitle: "BPI Danantara", topDesc: "Badan Pengelola", midTitle: "Dewan Komisaris", midDesc: "Pengawas", botBadge: "Pelaksana", botTitle: "Direksi PT Perminas", botDesc: "Mengelola seluruh klaster" };
        break;
                      ...
                      <div key={block.id} className="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden relative">
                        ...
                        <div className="p-5 space-y-4">
                          ...
                          {block.type === 'dashboard' && (
                            <div className="space-y-3"><div className="grid grid-cols-3 gap-2 bg-slate-50 p-2 rounded-lg border border-slate-200"><div className="space-y-1"><label className="text-[9px] font-bold text-slate-400 block">Kategori Atas</label><input type="text" value={bData.section || ''} onChange={e => updateBlockData(block.id, { ...bData, section: e.target.value })} placeholder="Kategori Atas" className="w-full text-xs border border-slate-200 rounded p-1 bg-white" /></div><div className="space-y-1"><label className="text-[9px] font-bold text-slate-400 block">Headline Penutup</label><input type="text" value={bData.title || ''} onChange={e => updateBlockData(block.id, { ...bData, title: e.target.value })} placeholder="Headline" className="w-full text-xs font-bold border border-slate-200 rounded p-1 bg-white" /></div><div className="space-y-1"><label className="text-[9px] font-bold text-slate-400 block">Label Badge</label><input type="text" value={bData.badge || ''} onChange={e => updateBlockData(block.id, { ...bData, badge: e.target.value })} placeholder="Badge Kanan" className="w-full text-xs border border-slate-200 rounded p-1 text-[#C5A059] bg-white" /></div></div><div className="grid grid-cols-1 sm:grid-cols-3 gap-2">{[1, 2, 3].map((num) => { const mKey = `metric${num}`; const mObj = bData[mKey] || {}; return (<div key={num} className="border border-slate-200 p-2 rounded bg-slate-50 space-y-1.5"><span className="text-[8px] font-bold text-[#0B4028] block">METRIK {num}</span><div className="space-y-1"><label className="text-[8px] text-slate-400 block">Label</label><input type="text" value={mObj.label || ''} onChange={e => updateBlockData(block.id, { ...bData, [mKey]: { ...mObj, label: e.target.value } })} placeholder="Label" className="w-full text-xs font-bold border border-slate-200 rounded p-1 bg-white" /></div><div className="grid grid-cols-2 gap-1"><div className="space-y-1"><label className="text-[8px] text-slate-400 block">Angka</label><input type="text" value={mObj.value || ''} onChange={e => updateBlockData(block.id, { ...bData, [mKey]: { ...mObj, value: e.target.value } })} placeholder="Nilai" className="w-full text-xs font-black border border-slate-200 rounded p-1 text-center bg-white" /></div><div className="space-y-1"><label className="text-[8px] text-slate-400 block">Satuan</label><input type="text" value={mObj.unit || ''} onChange={e => updateBlockData(block.id, { ...bData, [mKey]: { ...mObj, unit: e.target.value } })} placeholder="Unit" className="w-full text-xs border border-slate-200 rounded p-1 text-center bg-white text-slate-500" /></div></div><div className="space-y-1"><label className="text-[8px] text-slate-400 block">Sub-keterangan</label><input type="text" value={mObj.subtitle || ''} onChange={e => updateBlockData(block.id, { ...bData, [mKey]: { ...mObj, subtitle: e.target.value } })} placeholder="Sub-judul" className="w-full text-[10px] border border-slate-200 rounded p-1 bg-white" /></div><div className="space-y-1"><label className="text-[8px] text-slate-400 block">Deskripsi Detail</label><input type="text" value={mObj.desc || ''} onChange={e => updateBlockData(block.id, { ...bData, [mKey]: { ...mObj, desc: e.target.value } })} placeholder="Ket..." className="w-full text-[9px] border border-slate-200 rounded p-1 bg-white" /></div></div>); })}</div><div className="grid grid-cols-3 gap-2 pt-1 border-t border-slate-200"><div className="space-y-1"><label className="text-[9px] font-bold text-slate-400 block">Catatan Bawah</label><input type="text" value={bData.disclaimerTitle || ''} onChange={e => updateBlockData(block.id, { ...bData, disclaimerTitle: e.target.value })} placeholder="Judul Catatan Bawah" className="w-full text-xs font-bold border border-slate-200 rounded p-1 bg-white" /></div><div className="space-y-1"><label className="text-[9px] font-bold text-slate-400 block">Isi Catatan</label><input type="text" value={bData.disclaimerDesc || ''} onChange={e => updateBlockData(block.id, { ...bData, disclaimerDesc: e.target.value })} placeholder="Penjelasan regulasi..." className="w-full text-xs border border-slate-200 rounded p-1 bg-white" /></div><div className="space-y-1"><label className="text-[9px] font-bold text-slate-400 block">Tombol Aksi</label><input type="text" value={bData.buttonText || ''} onChange={e => updateBlockData(block.id, { ...bData, buttonText: e.target.value })} placeholder="Teks Tombol Unduh" className="w-full text-xs border border-slate-200 rounded p-1 text-[#0B4028] font-bold bg-white" /></div></div></div>
                          )}
                          {block.type === 'cta' && (
                          ...
                          {block.type === 'leadership_team' && (
                            <div className="space-y-4">
                              ...
                              <div className="bg-slate-50 p-2 rounded-lg border border-slate-200 space-y-2">
                                <div className="flex justify-between items-center">
                                  <span className="text-[9px] font-bold text-[#0B4028] uppercase">Dewan Komisaris</span>
